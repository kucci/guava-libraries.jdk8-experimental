...
abstract class AbstractMapBasedMultiset<E> extends AbstractMultiset<E>
  ...
  public void forEach(ObjIntConsumer<? super E> consumer) {
    ...
    backingMap.forEach((elem, count) -> consumer.accept(elem, count.get()));
  }

  Entry<E> wrapMapEntry(Map.Entry<E, Count> mapEntry) {
  ...
  Stream<Entry<E>> entryStream() {
    return backingMap.entrySet().stream().map(this::wrapMapEntry);
  }
