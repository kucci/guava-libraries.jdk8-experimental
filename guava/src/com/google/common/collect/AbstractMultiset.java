...
abstract class AbstractMultiset<E> extends AbstractCollection<E>
  ...
  @Override public Stream<E> stream() {
    return entrySet().stream().flatMap(
        entry -> Collections.nCopies(entry.getCount(), entry.getElement()).stream());
  }
  ...
  @Override public boolean removeIf(Predicate<? super E> predicate) {
    return elementSet().removeIf(predicate);
  }
  ...
  Spliterator<Entry<E>> entrySpliterator() {
    return Spliterators.spliterator(entryIterator(), distinctElements(),
        Spliterator.DISTINCT | Spliterator.NONNULL);
  }

  Stream<Entry<E>> entryStream() {
  ...
  class EntrySet extends Multisets.EntrySet<E> {
    ...
    @Override public Spliterator<Entry<E>> spliterator() {
      return entrySpliterator();
    }

    @Override public Stream<Entry<E>> stream() {
      return entryStream();
    }

    @Override public Stream<Entry<E>> parallelStream() {
      return stream().parallel();
    }
