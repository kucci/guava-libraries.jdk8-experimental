  ...
  static <R, C, V> RegularImmutableTable<R, C, V> forCells(
    ...
    if (rowComparator != null) {
      if (columnComparator != null) {
        cells.sort(comparing(Cell<R, C, V>::getRowKey, rowComparator)
            .thenComparing(Cell<R, C, V>::getColumnKey, columnComparator));
      } else {
        cells.sort(comparing(Cell<R, C, V>::getRowKey, rowComparator));
      }
    } else if (columnComparator != null) {
      cells.sort(comparing(Cell<R, C, V>::getColumnKey, columnComparator));
    }
