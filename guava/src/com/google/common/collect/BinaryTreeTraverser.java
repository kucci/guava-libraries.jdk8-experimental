...
public abstract class BinaryTreeTraverser<T> extends TreeTraverser<T> {
  ...
  private static <T> void ifPresent(Optional<T> optional, Consumer<? super T> consumer) {
  ...
  @Override
  Consumer<T> forEachPreOrder(Consumer<? super T> consumer) {
    checkNotNull(consumer);
    return new Consumer<T>() {
      @Override public void accept(T t) {
        consumer.accept(checkNotNull(t));
        ifPresent(leftChild(t), this);
        ifPresent(rightChild(t), this);
      }
    };
  }
  ...
  @Override
  Consumer<T> forEachPostOrder(Consumer<? super T> consumer) {
    checkNotNull(consumer);
    return new Consumer<T>() {
      @Override public void accept(T t) {
        checkNotNull(t);
  ...
  public final FluentIterable<T> inOrderTraversal(final T root) {
    ...
    return new FluentIterable<T>() {
      ...
      public void forEach(Consumer<? super T> consumer) {
        forEachInOrder(consumer).accept(root);
      }
    };
  }

  Consumer<T> forEachInOrder(Consumer<? super T> consumer) {
    checkNotNull(consumer);
    return new Consumer<T>() {
      @Override public void accept(T t) {
        checkNotNull(t);
    ...
    protected T computeNext() {
      while (!stack.isEmpty()) {
        ...
        if (hasExpandedLeft.get(stack.size() - 1)) {
          ...
          ifPresent(rightChild(node), stack::addLast);
          return node;
        } else {
          hasExpandedLeft.set(stack.size() - 1);
          ifPresent(leftChild(node), stack::addLast);
        }
      }
      return endOfData();
    }
