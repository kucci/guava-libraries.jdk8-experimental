...
abstract class AbstractMultimap<K, V> implements Multimap<K, V> {
  ...
  private Spliterator<Entry<K, V>> entrySpliterator() {
    return Spliterators.spliterator(entryIterator(), size(), 0);
  }
  ...
  class Values extends AbstractCollection<V> {
    ...
    @Override public void forEach(Consumer<? super V> consumer) {
      ...
      AbstractMultimap.this.forEach((key, value) -> consumer.accept(value));
    }
  ...
  Stream<V> valueStream() {
    return entryStream().map(Entry<K, V>::getValue);
  }
