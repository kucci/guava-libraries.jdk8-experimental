...
public final class Maps {
   ...
   *   BiFunction<String, Boolean, String> flagPrefixer =
   ...
   *
   * <p><b>Warning:</b> This method assumes that for any instance {@code k} of
   * {@code BiFunction} key type {@code K}, {@code k.equals(k2)} implies
   * that {@code k2} is also of type {@code K}. Using an {@code
   * BiFunction} key type for which this may not hold, such as {@code
   ...
   * {@link Maps#transformEntries(Map, BiFunction)}.
   *
   ...
   * @since 7.0
   */
  public interface EntryTransformer<K, V1, V2> extends BiFunction<K, V1, V2> {
    /**
     * Determines an output value based a key-value pair. This method is
    ...
    /**
     * @deprecated Provided for compatibility with {@link BiFunction}.  Please
     *     use {@code transformEntry} or transition fully over to
     *     {@code BiFunction}.
     */
    @Override
    @Deprecated
    default V2 apply(@Nullable K key, @Nullable V1 value) {
  ...
  static <K, V1, V2> Function<V1, V2> asValueToValueFunction(
      final BiFunction<? super K, V1, V2> transformer, final K key) {
    checkNotNull(transformer);
    return v1 -> transformer.apply(key, v1);
  }
  ...
  static <K, V1, V2> Function<Entry<K, V1>, V2> asEntryToValueFunction(
      final BiFunction<? super K, ? super V1, V2> transformer) {
    checkNotNull(transformer);
    return entry -> transformer.apply(entry.getKey(), entry.getValue());
  }
  ...
  static <V2, K, V1> Entry<K, V2> transformEntry(
      final BiFunction<? super K, ? super V1, V2> transformer, final Entry<K, V1> entry) {
    checkNotNull(transformer);
    checkNotNull(entry);
    return new AbstractMapEntry<K, V2>() {
      ...
      @Override
      public V2 getValue() {
        return transformer.apply(entry.getKey(), entry.getValue());
      }
    };
  }
  ...
  static class TransformedEntriesMap<K, V1, V2>
    ...
    @Override public V2 remove(Object key) {
      return fromMap.containsKey(key)
          ? transformer.apply((K) key, fromMap.remove(key))
          : null;
    }
  ...
  static final class FilteredEntryBiMap<K, V> extends FilteredEntryMap<K, V>
    ...
    private static <K, V> Predicate<Entry<V, K>> inversePredicate(
      ...
      return input -> forwardPredicate.test(Maps.immutableEntry(input.getValue(), input.getKey()));
    }
  ...
  public static <K, V> NavigableMap<K, V> synchronizedNavigableMap(
    ...
    return Collections.synchronizedNavigableMap(navigableMap);
  }
