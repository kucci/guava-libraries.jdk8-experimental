...
public abstract class ImmutableList<E> extends ImmutableCollection<E>
  ...
  /**
   * Returns a {@code Collector} that collects elements into an {@code ImmutableList}.
   */
  public static <E> Collector<E, ?, ImmutableList<E>> toImmutableList() {
    return Collectors.collectingAndThen(Collectors.<E>toList(), ImmutableList::copyOf);
  }
  ...
  // A default Spliterator implementation based on efficient random access.
  private class Splitr implements Spliterator<E> {
    ...
    @Override public boolean tryAdvance(Consumer<? super E> consumer) {
      if (offset < fence) {
        consumer.accept(get(offset++));
        return true;
      } else {
        return false;
      }
    ...
    @Override @Nullable public Spliterator<E> trySplit() {
      int lo = offset;
      int mid = (lo + fence) >>> 1;
      if (lo < mid) {
        offset = mid;
        return new Splitr(lo, mid);
      } else {
        return null;
      }
    }
