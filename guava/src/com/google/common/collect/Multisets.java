...
public final class Multisets {
  ...
  private static final class FilteredMultiset<E> extends AbstractMultiset<E> {
    ...
    Set<Entry<E>> createEntrySet() {
      return Sets.filter(unfiltered.entrySet(), entry -> predicate.test(entry.getElement()));
    }

    @Override public void forEach(ObjIntConsumer<? super E> consumer) {
      checkNotNull(consumer);
      unfiltered.forEach((element, count) -> {
        if (predicate.test(element)) {
          consumer.accept(element, count);
        }
      });
    }
  ...
  abstract static class ElementSet<E> extends Sets.ImprovedAbstractSet<E> {
    ...
    @Override public void forEach(Consumer<? super E> consumer) {
      checkNotNull(consumer);
      multiset().forEach((element, count) -> consumer.accept(element));
    }

    @Override public Iterator<E> iterator() {
      return Iterators.transform(multiset().entrySet().iterator(), Entry<E>::getElement);
    }
  ...
  private static final Ordering<Entry<?>> DECREASING_COUNT_ORDERING =
      Ordering.comparingInt(Entry<?>::getCount).reverse();
