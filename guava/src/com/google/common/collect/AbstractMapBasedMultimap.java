...
abstract class AbstractMapBasedMultimap<K, V> extends AbstractMultimap<K, V>
  ...
  public Collection<V> replaceValues(@Nullable K key, Iterable<? extends V> values) {
    ...
    Collection<V> collection = map.computeIfAbsent(key, this::createCollection);
  ...
  public void clear() {
    ...
    map.values().forEach(Collection<V>::clear);
    map.clear();
    totalSize = 0;
  }
  ...
  private class WrappedCollection extends AbstractCollection<V> {
    ...
    @Override public Spliterator<V> spliterator() {
      refreshIfEmpty();
      return delegate.spliterator(); // unmodifiable
    }

    @Override public Stream<V> stream() {
      refreshIfEmpty();
      return delegate.stream(); // unmodifiable
    }

    @Override public Stream<V> parallelStream() {
      refreshIfEmpty();
      return delegate.parallelStream(); // unmodifiable
    }
    ...
    @Override public boolean removeIf(Predicate<? super V> predicate) {
      checkNotNull(predicate);
      int oldSize = size();  // calls refreshIfEmpty
      boolean changed = delegate.removeIf(predicate);
      if (changed) {
        int newSize = delegate.size();
        totalSize += (newSize - oldSize);
        removeIfEmpty();
  ...
  private class KeySet extends Maps.KeySet<K, Collection<V>> {
    ...
    @Override public Stream<K> stream() {
      return map().keySet().stream();
    }

    @Override public Stream<K> parallelStream() {
      return map().keySet().parallelStream();
    }

    @Override public void forEach(Consumer<? super K> consumer) {
  ...
  Stream<V> valueStream() {
    return map.values().stream().flatMap(Collection<V>::stream);
  }
  ...
  private class AsMap extends ImprovedAbstractMap<K, Collection<V>> {
    ...
    @Override public void forEach(BiConsumer<? super K, ? super Collection<V>> consumer) {
      checkNotNull(consumer);
      submap.forEach((key, valueCollection) ->
          consumer.accept(key, wrapCollection(key, valueCollection)));
    }
    ...
    class AsMapEntries extends Maps.EntrySet<K, Collection<V>> {
      ...
      @Override public Stream<Map.Entry<K, Collection<V>>> stream() {
        return submap.entrySet().stream().map(AsMap.this::wrapEntry);
      }

      @Override public Stream<Map.Entry<K, Collection<V>>> parallelStream() {
        return submap.entrySet().parallelStream().map(AsMap.this::wrapEntry);
      }
