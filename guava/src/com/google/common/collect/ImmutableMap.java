...
public abstract class ImmutableMap<K, V> implements Map<K, V>, Serializable {
  ...
  static <V> BinaryOperator<V> rejectCollisions() {
    return (v1, v2) -> {
      throw new IllegalStateException("Multiple values for the same key: " + v1 + ", " + v2);
    };
  }

  /**
   * Returns a {@code Collector} that accumulates entries into an {@code ImmutableMap}, where
   * the keys and values are the results of applying the provided functions to the input elements.
   *
   * <p>If more than one input element maps to the same key, the {@code Collector} will throw an
  ...
  // TODO(user): consider a version which accepts Entries directly
  // TODO(user): consider a version which merges multiple values for a key
  ...
  @Deprecated
  @Override
  public final V remove(ImmutableMap<K, V> this, Object o) {
    throw new UnsupportedOperationException();
  }
