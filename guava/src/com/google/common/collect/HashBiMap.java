...
public final class HashBiMap<K, V> extends AbstractMap<K, V> implements BiMap<K, V>, Serializable {
  ...
  /**
   * Efficient Spliterator implementation for {@code HashBiMap}.
   */
  private class Splitr<T> implements Spliterator<T> {
    /*
     * Invariants:
     *
     * current is in the row hashTableKToV[offset - 1]
     * 0 <= offset <= fence
     */
    @Nullable
    private BiEntry<K, V> current;
    private int offset;
    private int knownSize; // -1 if unknown
    private final int fence;
    private final Function<? super BiEntry<K, V>, T> function;

    Splitr(Function<? super BiEntry<K, V>, T> function) {
      this.current = null;
      this.offset = 0;
      this.knownSize = size();
      this.fence = hashTableKToV.length;
      this.function = function;
    }

    private Splitr(BiEntry<K, V> current, int offset, int fence,
        Function<? super BiEntry<K, V>, T> function) {
    ...
    public int characteristics() {
      return Spliterator.DISTINCT | ((knownSize == -1) ? 0 : Spliterator.SIZED);
    }

    @Override
    public long estimateSize() {
      return (knownSize == -1) ? (fence - offset) : knownSize;
    }

    @Override
    public boolean tryAdvance(Consumer<? super T> consumer) {
      while (current == null && offset < fence) {
        current = hashTableKToV[offset++];
      }
      if (current != null) {
        consumer.accept(function.apply(current));
        if (knownSize != -1) {
          knownSize--;
        }
        current = current.nextInKToVBucket;
        return true;
      }
      return false;
    }
    ...
    public Spliterator<T> trySplit() {
      int mid = (offset + fence) >>> 1;
      if (mid < fence) {
        Spliterator<T> result = new Splitr(current, offset, mid, function);
        this.current = hashTableKToV[mid];
        this.offset = mid + 1;
        this.knownSize = -1;
        return result;
      } else {
  ...
  private final class KeySet extends Maps.KeySet<K, V> {
    ...
    public Spliterator<K> spliterator() {
      return new Splitr<>(Entry<K, V>::getKey);
    }
  ...
  private final class Inverse extends AbstractMap<V, K> implements BiMap<V, K>, Serializable {
    ...
    private final class InverseKeySet extends Maps.KeySet<V, K> {
      ...
      public Spliterator<V> spliterator() {
        return new Splitr<>(Entry<K, V>::getValue);
      }
    }
    ...
    public Set<Entry<V, K>> entrySet() {
      return new Maps.EntrySet<V, K>() {
        ...
        public Spliterator<Entry<V, K>> spliterator() {
          return new Splitr<>(entry -> Maps.immutableEntry(entry.getValue(), entry.getKey()));
        }
