...
final class ImmutableMapKeySet<K, V> extends ImmutableSet<K> {
  ...
  public UnmodifiableIterator<K> iterator() {
    return Iterators.transform(map.entrySet().iterator(), Entry<K, V>::getKey);
  }
  ...
  public Stream<K> stream() {
    return map.entrySet().stream().map(Entry<K, V>::getKey);
  }

  @Override
  public Stream<K> parallelStream() {
    return map.entrySet().parallelStream().map(Entry<K, V>::getKey);
  }
