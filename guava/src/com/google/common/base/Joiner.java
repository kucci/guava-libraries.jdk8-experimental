...
public class Joiner {
  ...
  @Nullable
  StringBuilder collectorJoin(@Nullable StringBuilder builder, @Nullable Object part) {
    if (builder == null) {
      return new StringBuilder(toString(part));
    } else {
      return builder.append(separator).append(toString(part));
    }
  }

  /**
   * Returns a {@code Collector} that can be used to join elements of a {@code Stream}.
   */
  public Collector<Object, ?, String> asCollector() {
    // Yes, I used a single-element array as a mutable holder.  Shoot me.
    return Collector.of(
        () -> new StringBuilder[1],
        (holder, part) -> {
          holder[0] = collectorJoin(holder[0], part);
        },
        (holder1, holder2) -> {
          if (holder1[0] == null) {
            return holder2;
            ...
            holder1[0].append(separator).append(holder2[0]);
            return holder1;
          }
        },
        holder -> {
  ...
  public Joiner skipNulls() {
    return new Joiner(this) {
      ...
      @Nullable
      StringBuilder collectorJoin(@Nullable StringBuilder builder, @Nullable Object part) {
        return (part == null) ? builder : super.collectorJoin(builder, part);
      }
