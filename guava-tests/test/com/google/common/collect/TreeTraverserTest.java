...
public class TreeTraverserTest extends TestCase {
  ...
  static String forEachOrder(Iterable<Tree> iterable) {
    StringBuilder builder = new StringBuilder();
    iterable.forEach(t -> builder.append(t.value));
    return builder.toString();
  }
  ...
  static String binaryForEachOrder(Iterable<BinaryTree> iterable) {
    StringBuilder builder = new StringBuilder();
    iterable.forEach(t -> builder.append(t.value));
    return builder.toString();
  }
  ...
  public void testPreOrder() {
    ...
    ASSERT.that(forEachOrder(ADAPTER.preOrderTraversal(h))).is("hdabcegf");
    ASSERT.that(binaryIterationOrder(BIN_ADAPTER.preOrderTraversal(bd))).is("dbacefg");
    ASSERT.that(binaryForEachOrder(BIN_ADAPTER.preOrderTraversal(bd))).is("dbacefg");
  }

  public void testPostOrder() {
    ASSERT.that(iterationOrder(ADAPTER.postOrderTraversal(h))).is("abcdefgh");
    ASSERT.that(forEachOrder(ADAPTER.postOrderTraversal(h))).is("abcdefgh");
    ASSERT.that(binaryIterationOrder(BIN_ADAPTER.postOrderTraversal(bd))).is("acbgfed");
    ASSERT.that(binaryForEachOrder(BIN_ADAPTER.postOrderTraversal(bd))).is("acbgfed");
  }

  public void testBreadthOrder() {
    ASSERT.that(iterationOrder(ADAPTER.breadthFirstTraversal(h))).is("hdegabcf");
    ASSERT.that(forEachOrder(ADAPTER.breadthFirstTraversal(h))).is("hdegabcf");
    ASSERT.that(binaryIterationOrder(BIN_ADAPTER.breadthFirstTraversal(bd))).is("dbeacfg");
    ASSERT.that(binaryForEachOrder(BIN_ADAPTER.breadthFirstTraversal(bd))).is("dbeacfg");
  }
