...
public class ImmutableMapTest extends TestCase {
  ...
  public static class CreationTests extends TestCase {
    ...
    public void testCollector() {
      BiPredicate<ImmutableMap<String, Integer>, ImmutableMap<String, Integer>> equiv =
          Equivalence.equals().onResultOf(
                  (ImmutableMap<String, Integer> map) -> map.entrySet().asList())
              .and(Equivalence.equals());
      CollectorTester
          .of(ImmutableMap.toImmutableMap(
              Entry<String, Integer>::getKey, Entry<String, Integer>::getValue), equiv)
          .expectCollects(ImmutableMap.of())
          .expectCollects(ImmutableMap.of("foo", 3, "bar", 1),
              Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 1))
          .expectCollects(ImmutableMap.of("foo", 3, "bar", 1, "baz", 5, "quux", 5),
              Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 1),
