...
public class ImmutableSortedMapTest extends TestCase {
  ...
  public static class CreationTests extends TestCase {
    ...
    public void testCollector() {
      BiPredicate<ImmutableSortedMap<String, Integer>, ImmutableSortedMap<String, Integer>> equiv =
          Equivalence.equals().onResultOf(
                  (ImmutableSortedMap<String, Integer> map) -> map.entrySet().asList())
              .and(Equivalence.equals().onResultOf(SortedMap<String, Integer>::comparator))
              .and(Equivalence.equals());
      CollectorTester
          .of(ImmutableSortedMap.toImmutableSortedMap(Ordering.natural(),
              Entry<String, Integer>::getKey, Entry<String, Integer>::getValue), equiv)
          .expectCollects(ImmutableSortedMap.of())
          .expectCollects(ImmutableSortedMap.of("foo", 3, "bar", 1),
              Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 1))
          .expectCollects(ImmutableSortedMap.of("foo", 3, "bar", 1, "baz", 5, "quux", 5),
              Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 1),
              Maps.immutableEntry("baz", 5), Maps.immutableEntry("quux", 5));
    }

    public void testCollectorThrowsOnDuplicateKeys() {
      try {
        Stream.of(Maps.immutableEntry("foo", 3), Maps.immutableEntry("foo", 1))
            .collect(ImmutableSortedMap.toImmutableSortedMap(Ordering.natural(),
              Entry<String, Integer>::getKey, Entry<String, Integer>::getValue));
