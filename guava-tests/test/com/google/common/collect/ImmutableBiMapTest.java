...
public class ImmutableBiMapTest extends TestCase {
  ...
  public static class CreationTests extends TestCase {
    ...
    public void testCollector() {
      BiPredicate<ImmutableBiMap<String, Integer>, ImmutableBiMap<String, Integer>> equiv =
          Equivalence.equals().onResultOf(
                  (ImmutableBiMap<String, Integer> map) -> map.entrySet().asList())
              .and(Equivalence.equals());
      CollectorTester
          .of(ImmutableBiMap.toImmutableBiMap(
              Entry<String, Integer>::getKey, Entry<String, Integer>::getValue), equiv)
          .expectCollects(ImmutableBiMap.of())
          .expectCollects(ImmutableBiMap.of("foo", 3, "bar", 1),
              Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 1))
          .expectCollects(ImmutableBiMap.of("foo", 3, "bar", 1, "baz", 5, "quux", 4),
              Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 1),
              Maps.immutableEntry("baz", 5), Maps.immutableEntry("quux", 4));
    }
    ...
    public void testCollectorThrowsOnDuplicateValues() {
      try {
        Stream.of(Maps.immutableEntry("foo", 3), Maps.immutableEntry("bar", 3))
            .collect(ImmutableBiMap.toImmutableBiMap(
              Entry<String, Integer>::getKey, Entry<String, Integer>::getValue));
        fail("Expected IllegalStateException");
      } catch (IllegalStateException expected) {
